    ...
    (err, usuario) => {
      ...
      bcrypt.hash(dados.senha, saltRounds, (err, senhaHash) => {
        ...
        db.run(
          ...
          [dados.nomeWorkspace],
          function (err) {
            console.log("🔴 DB INSERT - Erro:", err); // ← ADICIONE
            console.log("🟢 DB INSERT - Sucesso, ID:", this.lastID); // ← ADICIONE
            if (err) {
              return callback(null, {
                sucesso: false,
            ...
            db.run(
              ...
              function (err) {
                ...
                callback(null, {
                  ...
                  usuario: {
                    //  retornando o nome do workspace junto com os dados do usuário
                    id: this.lastID,
                    email: dados.email,
                    workspaceId: workspaceId,
                    workspaceNome: dados.nomeWorkspace, // adiciona o nome do workspace como workspaceNome à resposta.
                  },
                });
              }
            );
